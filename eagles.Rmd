...
(qb.plot <- hc_params %>%
  ...
  hc_xAxis(categories = subset(qb.melt$variable, qb.melt$Name == "S.Bradford")) %>%
  hc_title(text = "Sam Bradford", align = alignment))
knitr::kable(qb, col.names = c("", "Passing Attempts", "Passing Yards", "Touchdown Passes", "Interceptions", "Fumbles", "Lost Fumbles", "Rushing Yards"))
```

#### Sanchez
```{r, echo=F}
(qb.plot <- hc_params %>%
  hc_add_series(name="Sanchez", data = subset(qb.melt$value, qb.melt$Name == "M.Sanchez"), type = plot.type, colorByPoint = T)  %>%
  hc_xAxis(categories = subset(qb.melt$variable, qb.melt$Name == "S.Bradford")) %>%
  hc_title(text = "Mark Sanchez", align = alignment))
knitr::kable(qb, col.names = c("", "Passing Attempts", "Passing Yards", "Touchdown Passes", "Interceptions", "Fumbles", "Lost Fumbles", "Rushing Yards"))
